# CoChem-KINETIC: Microkinetic Modeling & TST
...
Calculates macroscopic, temperature-dependent reaction rate constants.
...
Integrates Transition State Theory (TST) with Grimme's qRRHO approximation for low-frequency modes. Computes Eckart tunneling corrections and interfaces with MESMER for RRKM master-equation solving.
...
```bash
cochem-kinetic --reactant A.xyz --ts TS.xyz --temp-range 200:1000:50
```
...
pipeline.run(target_module="CoChem-KINETIC")
```
