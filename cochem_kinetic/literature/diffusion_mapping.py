...
class SmoluchowskiDiffusionEngine:
    def __init__(self) -> None:
        self.known_viscosities: dict[str, float] = {
            "water": 8.9e-4
        }
        
    def _get_solvent_viscosity(self, solvent: str) -> float:
        """
        Retrieves known solvent viscosity in Pa*s (kg/(m*s)).
        """
        try:
            return self.known_viscosities[solvent.lower()]
    ...
    def compute_diffusion_limit(self, radius_a_angstrom: float, radius_b_angstrom: float, temp: float = 298.15, solvent: str = "water") -> float:
        ...
        eta = self._get_solvent_viscosity(solvent)
        ...
        try:
            ...
            raise ValueError("Radius or viscosity cannot be zero.")
    ...
    def process_association_rate(self, raw_rate: float, radius_a: float, radius_b: float, state_dict: dict[str, typing.Any]) -> float:
        """
